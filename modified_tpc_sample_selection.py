...
def tpc_rp_sampling(features, all_indices, all_labels, budget, num_labeled, max_clusters=500):
    ...
    num_classes = 10
    selected_class_counts = np.zeros(num_classes, dtype=int)
    ...
    if K <= 50:
        ...
        kmeans = MiniBatchKMeans(n_clusters=K, n_init=1, max_iter=20, random_state=seed_value, batch_size=6144)
    ...
    # Step 1: Ensure at least one sample per class if possible
    while len(selected_samples) < min(budget, num_classes):
        underrepresented_classes = np.where(selected_class_counts == 0)[0]
        ...
        for c in range(K):
            ...
            if not contains_labeled:
                ...
                cluster_indices = np.array(clusters[c])
                cluster_positions = [index_to_position_all[idx] for idx in cluster_indices]
                cluster_labels_dist = np.bincount(remaining_labels[cluster_positions], minlength=num_classes)
                cluster_info.append((c, len(clusters[c]), cluster_labels_dist))

        if not valid_clusters:
            break

        # Find a cluster with an underrepresented class
        for c, size, label_dist in cluster_info:
            ...
            break  # No cluster with underrepresented class found
        ...
        cluster_labels_local = remaining_labels[positions]

        nn = NearestNeighbors(n_neighbors=20, metric='euclidean')
        nn.fit(cluster_features)
        D, _ = nn.kneighbors(cluster_features)
        typicality = 1 / (np.mean(D, axis=1) + 1e-6)

        unlabeled_mask = ~labeled_mask[[index_to_position_all[idx] for idx in cluster_indices]]
        if not unlabeled_mask.any():
            continue

        # Select sample from an underrepresented class
        typicality_unlabeled = typicality[unlabeled_mask]
        labels_unlabeled = cluster_labels_local[unlabeled_mask]
        underrepresented_mask = np.isin(labels_unlabeled, underrepresented_classes)
        if not underrepresented_mask.any():
            continue
        scores = typicality_unlabeled[underrepresented_mask]
        top_idx = np.argmax(scores)
        selected_idx = cluster_indices[unlabeled_mask][underrepresented_mask][top_idx]

        selected_samples.append(selected_idx)
        selected_class_counts[all_labels[index_to_position_all[selected_idx]]] += 1
        labeled_mask[index_to_position_all[selected_idx]] = True

    # Step 2: Fill remaining budget with balanced selection
    while len(selected_samples) < budget:
        ...
        ideal_per_class = len(selected_samples) / num_classes
        class_imbalance = ideal_per_class - selected_class_counts

        def cluster_score(c_info):
            c, size, label_dist = c_info
            imbalance_score = np.sum(np.maximum(class_imbalance * label_dist, 0))
            return imbalance_score / (size + 1e-6)

        cluster_info.sort(key=cluster_score, reverse=True)
        selected_clusters = [c for c, _, _ in cluster_info[:samples_to_select]]
        ...
        for target_cluster in selected_clusters:
            ...
            typicality_unlabeled = typicality[unlabeled_mask]
            labels_unlabeled = cluster_labels_local[unlabeled_mask]
            scores = typicality_unlabeled + 2.0 * np.maximum(class_imbalance[labels_unlabeled], 0)  # Increase weight of balancing class
            top_idx = np.argmax(scores)
